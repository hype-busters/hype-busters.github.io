// Epistemic Stance Survey - JavaScript Module
// BWS/MaxDiff implementation with Google Sheets integration.

// Survey Configuration will be loaded from config.js

const EPISTEMIC_STANCE_CONFIG = {
    studyId: 'epistemic-stance',
    methodMode: 'bws_maxdiff',
    responseFormatVersion: '1.0',
    dataBasePath: '/src/data/epistemic-stance',
    localStorageKey: 'epistemicStanceSurveyResponses',
    defaultAppsScriptUrl: 'https://script.google.com/macros/s/AKfycbxFG53hdkB0s4FkVM0gAe9N6cST04KPDfG8kQbzSAxVzhTZ1zQDNQt4cQsw8sIip07p7g/exec'
};

// Function to detect attention check questions by specific word combinations
function checkIfAttentionCheck(words) {
    // For epistemic stance Phase 1 we do not enforce predefined checks.
    const attentionCheckSets = window.SURVEY_CONFIG?.epistemicStance?.attentionCheckSets || [];
...
async function loadQuestionsFromSurvey(surveyNumber) {
    ...
    try {
        ...
        const csvPath = `${EPISTEMIC_STANCE_CONFIG.dataBasePath}/survey${surveyNumber}.csv?v=${Date.now()}`;
...
function getAppsScriptUrl() {
    ...
    return config.epistemicStance?.googleAppsScript?.url ||
        config.googleAppsScript?.url ||
...
function showInstructionsModal(surveyNumber) {
    ...
    const categoryInstructions = {
        'LIKELIHOOD_AND_CERTAINTY': {
            meaning: 'These expressions change how probable or certain a claim sounds.',
            examples: 'This research <strong>might</strong> have important implications for X.<br>It is <strong>likely</strong> that this research has important implications for X.<br>It is <strong>undeniably</strong> true that this research has important implications for X.',
            rating: 'Judge how strongly the expression raises or lowers certainty about the same claim.'
        },
        'EVIDENTIAL_BASIS': {
            meaning: 'These expressions signal the strength or clarity of evidence supporting the claim.',
            examples: 'It <strong>seems</strong> that this research has important implications for X.<br>The findings <strong>suggest</strong> important implications for X.<br>The results <strong>prove</strong> important implications for X.',
            rating: 'Judge how strongly the expression frames evidence as weak, moderate, or decisive.'
        },
        'AUTHORIAL_POSITIONING': {
            meaning: 'These expressions show how strongly the writer personally commits to the claim.',
            examples: 'We <strong>suppose</strong> this research has important implications for X.<br>We <strong>argue</strong> this research has important implications for X.<br>We <strong>assert</strong> this research has important implications for X.',
            rating: 'Judge how strongly the expression communicates author commitment.'
        },
        'FREQUENCY': {
            meaning: 'These expressions indicate how often or generally a claim is presented as true.',
            examples: 'These methods <strong>sometimes</strong> play an important role in X.<br>These methods <strong>usually</strong> play an important role in X.<br>These methods <strong>always</strong> play an important role in X.',
            rating: 'Judge how strongly the expression indicates low-to-high frequency or generality.'
        },
        'DEGREE': {
            meaning: 'These expressions indicate intensity or magnitude of the stated implication.',
            examples: 'The implications are <strong>slightly</strong> important.<br>The implications are <strong>moderately</strong> important.<br>The implications are <strong>extremely</strong> important.',
            rating: 'Judge how strongly the expression amplifies or softens claim intensity.'
        }
    };
    
    // Define which categories are in each survey
    const surveyCategories = {
        '1': ['LIKELIHOOD_AND_CERTAINTY'],
        '2': ['EVIDENTIAL_BASIS'],
        '3': ['AUTHORIAL_POSITIONING'],
        '4': ['FREQUENCY'],
        '5': ['DEGREE']
    };
...
function saveToLocalStorage() {
    ...
    // Get existing data
    const existingData = JSON.parse(localStorage.getItem(EPISTEMIC_STANCE_CONFIG.localStorageKey) || '[]');
    existingData.push(surveyData);
    
    // Save back to localStorage
    localStorage.setItem(EPISTEMIC_STANCE_CONFIG.localStorageKey, JSON.stringify(existingData));
...
function exportToExcel() {
    ...
    const filename = `Epistemic_Stance_Survey_${participantData.name.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}.xlsx`;
